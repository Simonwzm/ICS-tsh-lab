...
void eval(char *cmdline) 
{
    ...
    if ((tpid=fork()) == 0)
    {
        ...
        // if (bg) {  // Only redirect for background jobs
        ...
        // control should never be here
        printf("%s: Command not found", argv[0]);
        exit(-1);
    }
    
    Sigprocmask(SIG_BLOCK, &mask_all, NULL);
// manual malloc example
    // struct job_t* new_job = (struct job_t*)(malloc(sizeof(struct job_t)));
    // if (!new_job)
    // {
    //     printf("malloc new job fail\n");
    //     exit(-1);
    // }
    // strcpy(new_job->cmdline, cmdline);
    // new_job->cmdline = cmdline;
    // new_job->jid = term_pid;
    // new_job->pid = term_pid;
    // new_job->state = job_state;
    int job_state = bg ? BG : FG;
    if (addjob(jobs, tpid, job_state, cmdline))
    {
        // listjobs(jobs);
        // if (verbose)
        // {
        //     printf("add job success, BG? %d\n", bg);
        //     printf("tpid = %d, fg_pid = %d\n", tpid, fg_pid);
        // }
    }
    else
    {
    ...
    else
    {
        ...
        printf("[%d] (%d) %s", pid2jid(tpid), tpid, cmdline);
        Sigprocmask(SIG_SETMASK, &prev_one, NULL);
        fg_pid = -1;
	    fflush(stdout);
        return; 
    ...
    else if (argv[1][0]=='%')
    {
        ...
        if (getjobjid(jobs, jid))
        {
            ...
            if (next_state == FG)
            {
                ...
                char* cmd = getjobjid(jobs, jid)->cmdline;
                printf("[%d] (%d) %s", jid, fg_pid, cmd);
...
void waitfg(pid_t pid)
{
    ...
    while (getjobpid(jobs, pid) && fg_pid != -1)
    {
        // Sio_putest\n");
        Sigsuspend(&prev_one);
    }
    if (verbose)
        Sio_puts("back from forejob wait\n");
    return;
}
...
void sigchld_handler(int sig) 
{
    ...
    sigset_t prev;
    pid_t temp_pid;
    while ((temp_pid = waitpid(-1,  &status, WUNTRACED | WNOHANG)) > 0)
    {
        if (verbose)
        { 
            Sio_puts("waitpid return: ");
            Sio_putl(temp_pid);
            Sio_puts("\n");
        }
        if (WIFSTOPPED(status))
        {
            ...
            if (verbose)
                printf("found %d stopped\n", temp_pid);
            getjobpid(jobs, temp_pid)->state = ST;
    ...
    {
        // if (verbose)
        //     Sio_puts("interupted by a signal\n");
    }
    if (errno != EINTR && errno != ECHILD )
    {
        ...
        { 
        Sio_putl(errno);
        Sio_puts("\nWaitpid internal error\n");
        }
    }
    errno = olderrno;
    if (verbose)
        Sio_puts("Out from sigchld handler\n");
    return;
}
...
void sigint_handler(int sig) 
{
    ...
    if (fg_pid == -1)
    {
        ...
        Sigprocmask(SIG_SETMASK, &prev, NULL);
        if (verbose)
            Sio_puts("no foreground job to interupt\n");
        return;
    }
    else
    {
...
void listjobs(struct job_t *jobs) 
{
    ...
    // printf("list end with item num=%d\n", i);
}
/******************************
